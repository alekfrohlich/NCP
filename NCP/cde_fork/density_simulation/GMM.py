...
class GaussianMixture(BaseConditionalDensitySimulation):
    ...
    def __init__(
        self, n_kernels=5, ndim_x=1, ndim_y=1, means_std=1.5, random_seed=None
    ):

        self.random_state = np.random.RandomState(
            seed=random_seed
        )  # random state for sampling data
        self.random_state_params = np.random.RandomState(
            seed=20
        )  # fixed random state for sampling GMM params
      ...
      (x,y), x and y for later conditional draws """
        ...
        for i in range(n_kernels):
            self.gaussians.append(
                stats.multivariate_normal(mean=self.means[i,], cov=self.covariances[i])
            )
            self.gaussians_x.append(
                stats.multivariate_normal(
                    mean=self.means_x[i,], cov=self.covariances_x[i]
                )
            )
            self.gaussians_y.append(
                stats.multivariate_normal(
    ...
    def pdf(self, X, Y):
        ...
        P_y = np.stack(
            [self.gaussians_y[i].pdf(Y) for i in range(self.n_kernels)], axis=1
        )  # shape(X.shape[0], n_kernels)
        W_x = self._W_x(X)
    ...
    def cdf(self, X, Y):
        ...
        P_y = np.stack(
            [self.gaussians_y[i].cdf(Y) for i in range(self.n_kernels)], axis=1
        )  # shape(X.shape[0], n_kernels)
        W_x = self._W_x(X)
    ...
    def _W_x(self, X):
        ...
        if X.shape[0] == 1:
            w_p = np.stack(
                [
                    np.array([self.weights[i] * self.gaussians_x[i].pdf(X)])
            ...
            w_p = np.stack(
                [
                    self.weights[i] * self.gaussians_x[i].pdf(X)
                    for i in range(self.n_kernels)
                ],
                axis=1,
            )
