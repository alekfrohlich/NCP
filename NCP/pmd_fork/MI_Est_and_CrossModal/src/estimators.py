...
def js_fgan_lower_bound_obj(f):
    ...
    n = f.size(0)
    second_term = (torch.sum(F.softplus(f)) - torch.sum(F.softplus(f_diag))) / (n * (n - 1.0))
...
def log_density_ratio_mi(f):
    return torch.log(torch.clamp(f, min=1e-9)).diag().mean()
...
# train_type = density_ratio_fitting_obj(scores = forward(k, x, y))
#            = contrastive loss
#
# eval_type  = log_density_ratio_mi(f = scores)
#
# def log_density_ratio_mi(f):
#   return torch.log(torch.clamp(f, min=1e-4)).diag().mean()
#
# TL;DR, for DRF it returns:
# (mi, train_obj) = eval_val, train_val
def MI_Estimator(f, train_type="nwj_lower_bound_obj", eval_type="nwj_lower_bound_obj", **kwargs):
    ...
    if train_type == "nwj_lower_bound_obj" and eval_type == "direct_log_density_ratio_mi":
        eval_val = getattr(sys.modules[__name__], eval_type)(f - 1.0)
    elif eval_type == "tuba_lower_bound" or eval_type == "dv_clip_upper_lower_bound" or eval_type == "mine_lower_bound":
    ...
    # note that especially when we use JS to train, and use nwj to evaluate
        ...
        eval_val = getattr(sys.modules[__name__], eval_type)(f + 1.0, **kwargs)
    else:
...
def smile_lower_bound(f, alpha=1.0, clip=5.0):
    return MI_Estimator(
        f, train_type="js_fgan_lower_bound_obj", eval_type="dv_clip_upper_lower_bound", alpha=alpha, clip=clip
    )
...
def variational_f_nwj(f):
    return MI_Estimator(f, train_type="nwj_lower_bound_obj", eval_type="direct_log_density_ratio_mi")


## Proposed Method 1: Variational Representation of f-divergence (DV)
def variational_f_dv(f):
    return MI_Estimator(f, train_type="dv_upper_lower_bound_obj", eval_type="direct_log_density_ratio_mi")


## Proposed Method 1: Variational Representation of f-divergence (JS)
def variational_f_js(f):
    return MI_Estimator(f, train_type="js_fgan_lower_bound_obj", eval_type="direct_log_density_ratio_mi")
...
def density_matching(f):
    ...
    return MI_Estimator(f, train_type="density_matching_obj", eval_type="direct_log_density_ratio_mi")
...
def density_matching_lagrange(f):
    # we change the variables here (ratio -> log_ratio) because of non-negativity
    return MI_Estimator(f, train_type="density_matching_lagrange_obj", eval_type="direct_log_density_ratio_mi")
    # return MI_Estimator(f, train_type='density_matching_loss',
    #                    eval_type='log_density_ratio_mi')
...
def probabilistic_classifier(f):
    return MI_Estimator(f, train_type="probabilistic_classifier_obj", eval_type="probabilistic_classifier_eval")
...
def density_ratio_fitting(f):
    # return MI_Estimator(f, train_type='density_ratio_fitting_obj',
    #                   eval_type='log_density_ratio_mi')
    return MI_Estimator(f, train_type="density_ratio_fitting_obj", eval_type="log_density_ratio_mi")


def squared_mutual_information(f):
    return MI_Estimator(f, train_type="density_ratio_fitting_obj", eval_type="squared_mutual_information_eval")


def js_squared_mutual_information(f):
    return MI_Estimator(f, train_type="js_fgan_lower_bound_obj", eval_type="squared_mutual_information_from_log_eval")
...
def density_ratio_fitting_obj(f, l=0.0, relative_ratio=0.0):
    ...
    marg_term = (f_square.sum() - f_square.diag().sum()) / (n * (n - 1.0))

    if not l == 0.0:
        normalization_contraint = (f.sum() - f.diag().sum()) / (n * (n - 1.0))
    ...
    if not relative_ratio == 0.0:
        ...
        marg_term = (1.0 - relative_ratio) * marg_term
...
def density_matching_obj(f, l=1.0):
    ...
    n = f.size(0)
    marg_term = (f.exp().sum() - f.exp().diag().sum()) / (n * (n - 1.0))
...
def probabilistic_classifier_obj(f):
    ...
    Loss = -1.0 * criterion(logits, labels)
...
def probabilistic_classifier_eval(f):
    ...
    MI = torch.mean(torch.log((batch_size - 1) * joint_logits / (1.0 - joint_logits)))
