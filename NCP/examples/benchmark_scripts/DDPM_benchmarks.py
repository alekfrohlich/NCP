...
def cdf_from_sample(model:DDPM, x, ys, N=1000):
    samples, _ = model.sample(x, N, device=model.device)
    cdf = ecdf(samples.flatten())
    return cdf.cdf.evaluate(ys)

def pdf_from_sample(model:DDPM, x, ys, N=1000):
    samples, _ = model.sample(x, N, device=model.device)
    pdf = compute_marginal(bandwidth='scott').fit(samples)
    return pdf(ys)
...
def run_experiment(density_simulator, density_simulator_kwargs):
    ...
    filename = density_simulator().__class__.__name__ + '_DDPM_results.pkl'
    ...
    for n in tqdm(n_training_samples, desc='Training samples', total=len(n_training_samples)):
        ...
        for seed in tqdm(random_seeds, desc='Seed', total=len(random_seeds)):
            ...
            ddpm = DDPM(nn_model=ContextUnet(x_dim=X_train.shape[-1], y_dim=Y_train.shape[-1]), betas=(1e-4, 0.02),
                        n_T=400, device=device, drop_prob=0.1)
            ...
            ckpt_path = "checkpoints/" + density.__class__.__name__ + "_DDPM_" + str(n) + "_" + str(seed)
            ...
            best_model = torch.load(checkpoint_callback.dirpath + '/best_model.pt').to(device)
            best_model.eval()

            print('N epochs: {0}'.format(ddpm_module.current_epoch - early_stop.patience))
            print('Training loss: {0}'.format(ddpm_module.train_loss[-early_stop.patience]))
            print('Validation loss: {0}'.format(ddpm_module.val_loss[-early_stop.patience]))
